...
from multiprocessing import Pool ###allows for easy multithreading
...
#TODO this is where the training actually occurs (look to parallelise here)
    pool = Pool()
    results = pool.map(batch_training)
    pool.close()
    pool.join()
...
###TODO function that isolates the training of each individual batch
def batch_training(batch_index,
                   batch_start,
                   batch_end,
                   batches,
                   batch_ids,
    ...
    return (batch_outs, batch_start, batch_end)
